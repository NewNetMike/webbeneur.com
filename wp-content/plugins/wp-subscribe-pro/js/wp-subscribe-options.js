...
( function( $ ){
	$( document ).ready( function() {
		...
		$('.services_dropdown').change(function() {
			...
			var $namefield = $this.parent().siblings('.wp_subscribe_include_name_wrapper');
			var $thankyoufields = $this.parent().siblings('.wp_subscribe_thanks_page');
			...
			// Thanks Page option
	        if ($this.val() == 'mailchimp' || $this.val() == 'getresponse' || $this.val() == 'mailerlite' || $this.val() == 'benchmark' || $this.val() == 'constantcontact' || $this.val() == 'mailrelay' ) {
	        	$thankyoufields.show();
	        } else {
	        	$thankyoufields.hide();
	        }
		}).trigger('change');
		$('.thanks-page-field').change(function(event) {

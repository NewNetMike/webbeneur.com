...
function mts_inner_sidebar_metabox($post) {
    ...
    */
    ...
    echo '<option value="" '.selected('', $custom_sidebar).'>-- '.__('Default', 'steadyincome').' --</option>';
    
    // Exclude built-in sidebars
    $hidden_sidebars = array('sidebar', 'footer-first', 'footer-first-2', 'footer-first-3', 'footer-first-4', 'footer-second', 'footer-second-2', 'footer-second-3', 'footer-second-4', 'widget-header','shop-sidebar', 'product-sidebar','banner');    
    ...
    echo '<option value="mts_nosidebar" '.selected('mts_nosidebar', $custom_sidebar).'>-- '.__('No sidebar --', 'steadyincome').'</option>';    
    echo '</select><br />';
    
    // Select single layout (left/right sidebar)
    echo '<div class="mts_sidebar_location_fields">';
    echo '<label for="mts_sidebar_location_default" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_default" value=""'.checked('', $sidebar_location, false).'>'.__('Default side', 'steadyincome').'</label>';
    echo '<label for="mts_sidebar_location_left" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_left" value="left"'.checked('left', $sidebar_location, false).'>'.__('Left', 'steadyincome').'</label>';
    echo '<label for="mts_sidebar_location_right" style="display: inline-block; margin-right: 20px;"><input type="radio" name="mts_sidebar_location" id="mts_sidebar_location_right" value="right"'.checked('right', $sidebar_location, false).'>'.__('Right', 'steadyincome').'</label>';
    echo '</div>';
...
function mts_add_posttemplate_metabox() {
    add_meta_box(
        ...
        __('Template', 'steadyincome'),      // title
...
function mts_inner_posttemplate_metabox($post) {
    ...
    */
    ...
    echo '<option value="" '.selected('', $posttemplate).'>'.__('Default Post Template', 'steadyincome').'</option>';
    echo '<option value="parallax" '.selected('parallax', $posttemplate).'>'.__('Parallax Template', 'steadyincome').'</option>';
    echo '<option value="zoomout" '.selected('zoomout', $posttemplate).'>'.__('Zoom Out Effect Template', 'steadyincome').'</option>';
    echo '</select><br />';
}
...
function mts_add_postheader_metabox() {
    ...
    foreach ($screens as $screen) {
        add_meta_box(
            ...
            __('Header Animation', 'steadyincome'),    // title
...
function mts_inner_postheader_metabox($post) {
    ...
    */
    ...
    echo '<option value="" '.selected('', $postheader).'>'.__('None', 'steadyincome').'</option>';
    echo '<option value="parallax" '.selected('parallax', $postheader).'>'.__('Parallax Effect', 'steadyincome').'</option>';
    echo '<option value="zoomout" '.selected('zoomout', $postheader).'>'.__('Zoom Out Effect', 'steadyincome').'</option>';
    echo '</select><br />';
}

...
/*-----------------------------------------------------------------------------------*/
define( 'MTS_THEME_NAME', 'steadyincome' );
...
/*-----------------------------------------------------------------------------------*/
load_theme_textdomain( 'steadyincome', get_template_directory().'/lang' );
...
if ( !empty( $mts_options['translate'] )) {
    ...
    function mts_custom_translate( $translated_text, $text, $domain ) {
        if ( $domain == 'steadyincome' || $domain == 'nhp-opts' ) {
...
function mts_get_icons() {
    $icons = array(
        __( 'Web Application Icons', 'steadyincome' ) => array(
        ...
        __( 'Hand Icons', 'steadyincome' ) => array(
        ...
        __( 'File Type Icons', 'steadyincome' ) => array(
        ...
        __( 'Spinner Icons', 'steadyincome' ) => array(
            'circle-o-notch', 'cog', 'refresh', 'spinner'
        ),
        __( 'Form Control Icons', 'steadyincome' ) => array(
        ...
        __( 'Directional Icons', 'steadyincome' ) => array(
        ...
        __( 'Video Player Icons', 'steadyincome' ) => array(
        ...
        __( 'Brand Icons', 'steadyincome' ) => array(
        ...
        __( 'Medical Icons', 'steadyincome' ) => array(
...
function mts_after_setup_theme() {
    ...
    add_image_size( 'steadyincome-featured', 555, 305, true ); //featured
    add_image_size( 'steadyincome-blog', 224, 224, true ); //featured
    add_image_size( 'steadyincome-archive', 329, 230, true ); //featured
    add_image_size( 'steadyincome-featuredfull', 770, 300, true ); //featured full width
    add_image_size( 'steadyincome-widgetthumb', 70, 70, true ); //widget
    add_image_size( 'steadyincome-widgetfull', 330, 220, true ); //widget full width

    register_nav_menus( array(
      'primary-menu' => __( 'Primary', 'steadyincome' ),
      'secondary-menu' => __( 'Secondary', 'steadyincome' ),
      'mobile' => __( 'Mobile', 'steadyincome' )
    ) );
...
function magazine_wp_review_thumb_size() {
    ...
    add_image_size( 'wp_review_small', 70, 70, true );
}
...
function mts_get_featured_image($post_ID) {  
    ...
    if ($post_thumbnail_id) {  
        $post_thumbnail_img = wp_get_attachment_image_src($post_thumbnail_id, 'steadyincome-widgetfull');  
...
if (isset($_GET['activated']) && is_admin()){
    //homepage
    $home_page_title = __('Homepage','steadyincome');
    $home_page_content = '';
    ...
    //blog
    $blog_page_title = __('Blog','steadyincome');
    $blog_page_content = '';
    $blog_page_template = 'page.php';
    //don't change the code bellow, unless you know what you're doing
...
function mts_columns_head($defaults) {
    if (get_post_type() == 'post')
        $defaults['featured_image'] = __('Featured Image', 'steadyincome');
    return $defaults;  
}  
...
if (!empty($mts_options['mts_sticky_nav'])) {
    add_filter( 'body_class', 'mts_sticky_nav_class' );
    function mts_sticky_nav_class( $classes ) {
        $classes[] = 'mts-sticky-nav';
        return $classes;
    }
}
...
if ( function_exists( 'register_sidebar' ) ) {   
    function mts_register_sidebars() {
        ...
        register_sidebar( array(
            'name' => __('Sidebar','steadyincome'),
            'description'   => __( 'Default sidebar.', 'steadyincome' ),
            'id' => 'sidebar',
        ...
        register_sidebar( array(
            'name' => __( 'Home Subscribe Widget', 'steadyincome' ),
            'description'   => __( 'Add WP Subscribe Pro widget here.', 'steadyincome' ),
            'id' => 'banner',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'after_widget' => '</div>',
            'before_title' => '<h3 class="widget-title">',
        ...
        if ( !empty( $mts_options['mts_first_footer'] )) {
            ...
            register_sidebars( $mts_options['mts_first_footer_num'], array(
                'name' => __( 'Footer Widget %d', 'steadyincome' ),
                'description'   => __( 'Add widgets in the footer', 'steadyincome' ),
                'id' => 'footer-first',
                'before_widget' => '<div id="%1$s" class="widget %2$s">',
                'after_widget' => '</div>',
                'before_title' => '<h3 class="widget-title">',
        ...
        register_sidebar( array(
            'name' => __('Shop Page Sidebar', 'steadyincome' ),
            'description'   => __( 'Appears on Shop main page and product archive pages.', 'steadyincome' ),
            'id' => 'shop-sidebar',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'after_widget' => '</div>',
            'before_title' => '<h3 class="widget-title">',
        ...
        register_sidebar( array(
            'name' => __('Single Product Sidebar', 'steadyincome' ),
            'description'   => __( 'Appears on single product pages.', 'steadyincome' ),
            'id' => 'product-sidebar',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'after_widget' => '</div>',
            'before_title' => '<h3 class="widget-title">',
...
function mts_load_footer_scripts() {  
   ...
   // RTL
    if ( ! empty( $mts_options['mts_rtl'] ) ) {
        wp_enqueue_style( 'mts_rtl', get_template_directory_uri() . '/css/rtl.css', 'steadyincome-style' );
    }
    ...
    if( ! empty( $mts_options['mts_pagenavigation_type'] ) && $mts_options['mts_pagenavigation_type'] >= 2 /*&& !is_singular()*/ ) {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'autoLoad' => $autoload,
                'i18n_loadmore' => __('Load More Posts', 'steadyincome'),
                'i18n_loading' => __('Loading...', 'steadyincome'),
                'i18n_nomore' => __('No more posts.', 'steadyincome')
            )
        );
    }
...
function mts_enqueue_css() {
    ...
    wp_enqueue_style( 'steadyincome-stylesheet', get_stylesheet_uri() );
    // Slider
    ...
    $handle = 'steadyincome-stylesheet';
   ...
   // WooCommerce
    ...
    if ( $mts_sidebar_location != 'right' && ( $mts_options['mts_layout'] == 'sclayout' || $mts_sidebar_location == 'left' )) {
        ...
        if( isset( $mts_options['mts_social_button_position'] ) && $mts_options['mts_social_button_position'] == 'floating' ) {
            $mts_shareit_right = '@media screen and (min-width:1400px) { .shareit { margin: 0 760px 0; border-left: 0; } }';
        }
    }
        ...
        <div id="comment-<?php comment_ID(); ?>" itemscope itemtype="http://schema.org/UserComments">
            ...
            <div class="commentmetadata">
                <div class="commenttext" itemprop="commentText">
                    <div class="front-view-content"><?php comment_text() ?></div>
                </div>
    ...
    function mts_readmore() {
        ...
        <div class="readMore">
            <a href="<?php echo get_the_permalink(); ?>" title="<?php the_title(); ?>">
                <?php _e( 'Read More', 'steadyincome' ); ?> <i class="fa fa-angle-double-right"></i>
            </a>
        </div>
        <?php 
    }
...
function ajax_mts_search() {
    ...
    if ( !empty( $query ) && $search_query->have_posts() ) : 
            ...
            ?><li>
                <a href="<?php echo esc_url( get_the_permalink() ); ?>">
                    <?php if ( has_post_thumbnail() ) { ?>
                        <?php the_post_thumbnail( 'steadyincome-widgetthumb', array( 'title' => '' ) ); ?>
                    <?php } else { ?>
                        <img class="wp-post-image" src="<?php echo get_template_directory_uri() . '/images/nothumb-steadyincome-widgetthumb.png'; ?>" alt="<?php echo esc_attr( get_the_title() ); ?>"/>
                    <?php } ?>
                    <?php the_title(); ?>
                </a>
                <div class="meta">
        ...
        endwhile;
        echo '</ul>';
        echo '<div class="ajax-search-meta"><span class="results-count">'.$search_count.' '.__( 'Results', 'steadyincome' ).'</span><a href="'.esc_url( get_search_link( $query ) ).'" class="results-link">'.__('Show all results.', 'steadyincome' ).'</a></div>';
    else:
        echo '<div class="no-results">'.__( 'No results found.', 'steadyincome' ).'</div>';
    endif;
    wp_reset_postdata();
...
if ( mts_is_wc_active() ) {
    ...
    function mts_header_add_to_cart_fragment( $fragments ) {
        ...
        <a class="cart-contents" href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" title="<?php _e( 'View your shopping cart', 'steadyincome' ); ?>"><?php echo sprintf( _n( '%d item', '%d items', $woocommerce->cart->cart_contents_count, 'steadyincome' ), $woocommerce->cart->cart_contents_count );?> - <?php echo $woocommerce->cart->get_cart_total(); ?></a>
...
function mts_megamenu_thumbnails( $thumbnail_html, $post_id ) {
    ...
    if(has_post_thumbnail($post_id)):
        $thumbnail_html .= get_the_post_thumbnail($post_id, 'steadyincome-widgetfull', array('title' => ''));
    else:
        $thumbnail_html .= '<img src="'.get_template_directory_uri().'/images/nothumb-steadyincome-widgetfull.png" alt="'.__('No Preview', 'steadyincome').'"  class="wp-post-image" />';
    endif;
    $thumbnail_html .= '</a>';
...
{
    ?>
        <h3><?php _e('New User Profile Links', 'steadyincome'); ?></h3>

        <table class="form-table">
            <tr>
                <th><label for="facebook_profile"><?php _e('Facebook Profile', 'steadyincome'); ?></label></th>
            ...
            <tr>
                <th><label for="twitter_profile"><?php _e('Twitter Profile', 'steadyincome'); ?></label></th>
            ...
            <tr>
                <th><label for="google_profile"><?php _e('Google+ Profile', 'steadyincome'); ?></label></th>
...
function mts_admin_bar_link() {
    ...
    if( current_user_can( 'edit_theme_options' ) ) {
        $wp_admin_bar->add_menu( array(
            ...
            'title' => __( 'Theme Options', 'steadyincome' ),
...
function mts_in_post_wp_subscribe() {
    global $mts_options;
    ...
    the_widget(
        'wp_subscribe',
        array(
            'service' => $mts_options['mts_single_subscribe_service'],
            'include_name_field' => $mts_options['mts_single_subscribe_include_name_field'],
            'feedburner_id' => $mts_options['mts_single_subscribe_feedburner_id'],
            'mailchimp_api_key' => $mts_options['mts_single_subscribe_mailchimp_api_key'],
            'mailchimp_list_id' => $mts_options['mts_single_subscribe_mailchimp_list_id'],
            'aweber_list_id' => $mts_options['mts_single_subscribe_aweber_list_id'],

            'title' => $mts_options['mts_single_subscribe_title'],
            'text' => $mts_options['mts_single_subscribe_text'],
            'name_placeholder' => $mts_options['mts_single_subscribe_name_placeholder'],
            'email_placeholder' => $mts_options['mts_single_subscribe_email_placeholder'],
            'button_text' => $mts_options['mts_single_subscribe_button_text'],
            'success_message' => $mts_options['mts_single_subscribe_success_message'],
            'error_message' => $mts_options['mts_single_subscribe_error_message'],
            'already_subscribed_message' => $mts_options['mts_single_subscribe_already_subscribed_message'],
            'footer_text' => $mts_options['mts_single_subscribe_footer_text'],
        ),
        array(
            'before_widget' => '<div class="wp-subscribe-single wp-subscribe-shortcode">',
        )
    );
    return ob_get_clean();
}
...
function mts_is_wp_subscribe_active() {
    if ( is_multisite() ) {
        include_once( ABSPATH . 'wp-admin/includes/plugin.php' );

        if ( is_plugin_active( 'wp-subscribe-pro/wp-subscribe-pro.php' ) ) {
            return 'pro';
        } elseif ( is_plugin_active( 'wp-subscribe/wp-subscribe.php' ) ) {
            return 'free';
        }
    } else {
        if ( in_array( 'wp-subscribe-pro/wp-subscribe-pro.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            return 'pro';
        } elseif ( ( in_array( 'wp-subscribe/wp-subscribe.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) ) {
            return 'free';
        }
    }
...
function mts_add_admin_body_class( $classes ) {
    $screen = get_current_screen();
    $body_class = '';
    if ( 'appearance_page_theme_options' === $screen->id ) {
        $body_class = 'wp-subscribe-check';
        $subscribe = mts_is_wp_subscribe_active();
        if ( $subscribe ) {
            $body_class .= ' wp-subscribe-active-'.$subscribe;
        } else {
            $body_class .= ' wp-subscribe-active-none';
        }
    }
    return "$classes $body_class";
}
add_filter( 'admin_body_class', 'mts_add_admin_body_class' );

function mts_wp_subscribe_option_scripts(){
    $screen = get_current_screen();
    if ( 'appearance_page_theme_options' === $screen->id ) {
        wp_enqueue_script(
            'si-options',
            get_template_directory_uri().'/js/si-options.js',
            array('jquery'),
            MTS_THEME_VERSION,
            true
        );
...
add_action( 'admin_enqueue_scripts', 'mts_wp_subscribe_option_scripts' );
...
function mts_correct_homepage_sections_import( $item, $key, $data ) {
    ...
    if ( !in_array( $key, array( 'mts_books_image', 'mts_footer_slider' ) ) ) return $item;

    $new_item = $item;

    if ( 'mts_books_image' === $key ) {

        foreach( $item as $i => $image ) {

            $id = $image['mts_book_image'];

            if ( is_numeric( $id ) ) {

                if ( array_key_exists( $id, $data['posts'] ) ) {

                    $new_item[ $i ]['mts_book_image'] = $data['posts'][ $id ];
                }
                ...
                if ( array_key_exists( $id, $data['image_urls'] ) ) {
                    ...
                    $new_item[ $i ]['mts_book_image'] = $data['image_urls'][ $id ];
                }
            }
        }

    } else { // mts_footer_slider

        foreach( $item as $i => $image ) {

            $id = $image['mts_footer_slider_image'];

            if ( is_numeric( $id ) ) {

                if ( array_key_exists( $id, $data['posts'] ) ) {

                    $new_item[ $i ]['mts_footer_slider_image'] = $data['posts'][ $id ];
                }
                ...
                if ( array_key_exists( $id, $data['image_urls'] ) ) {
                    ...
                    $new_item[ $i ]['mts_footer_slider_image'] = $data['image_urls'][ $id ];
                }
            }
        }
    }
...
$mts_featured_posts = array();
if ( ! empty( $mts_options['mts_featured_post_dedup'] ) ) {
    function mts_found_posts( $found_posts, $query ) {
        global $mts_featured_posts;
        if ( is_front_page() && ! $query->is_main_query() ) {
            foreach ( $query->posts as $post ) {
                $mts_featured_posts[] = (int) $post;
            }
        }
        return $found_posts;
    }
    add_filter( 'found_posts', 'mts_found_posts', 10, 2 );
}

add_action( 'wp_footer', 'om_dynamic_monsterlink_wp_menu' );
/**
 * Dynamically adds the required attributes to the menu item #4632 link to trigger the specified MonsterLink
 * IMPORTANT - replace '4632' below with the ID of your specific menu item
 */
function om_dynamic_monsterlink_wp_menu() {
  echo '<script type="text/javascript">jQuery(document).ready(function($){
    $("#menu-item-397 a").attr("class", "manual-optin-trigger");
    $("#menu-item-397 a").attr("data-optin-slug", "rcfiurelakkhomj9");
  });</script>';
}